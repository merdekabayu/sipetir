...
set kecamatan=gmt\kecamatan-malut.dat
...
for /f "delims=*" %%a in ('gawk "BEGIN {print %evlat%-0.4}" ') do set scalelat=%%a
for /f "delims=*" %%a in ('gawk "BEGIN {print %evlon%+0.6}" ') do set scalelon=%%a
...
gawk "{ print $1+0.05,$2,$3,$4,$5,$6,$7}" %kota_besar_malut% | pstext -R -JM -O -K >> %psfile%
gawk "{ print $1+0.02,$2,$3-2,$4,$5,$6,$7}" %kota_malut% | pstext -R -JM -O -K >> %psfile%
gawk "{ print $1+0.02,$2,$3-2,$4,$5,$6,$7}" %kota_sulut% | pstext -R -JM -O -K >> %psfile%
gawk "{ print $1+0.02,$2,$3-2,$4,$5,$6,$7,$8,$9,$10,$11}" %kecamatan% | pstext -R -JM -O -K >> %psfile%

psbasemap -J -R%REGION% -B%AXIS% --MAP_LABEL_OFFSET=0.2 --FONT_ANNOT_PRIMARY=9p --FONT_LABEL=11p,Heveltica --MAP_FRAME_TYPE=inside --MAP_FRAME_WIDTH=0.1p -P -K -O -V -Lf%scalelon%/%scalelat%/1/20+lKM  >> %psfile%
