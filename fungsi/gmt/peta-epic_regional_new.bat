...
for /f "delims=*" %%a in ('gawk "BEGIN {print %evlat%-1.6}" ') do set scalelat=%%a
for /f "delims=*" %%a in ('gawk "BEGIN {print %evlon%+1.8}" ') do set scalelon=%%a

echo %evlon% %evlat% > %parameter%
grdimage gmt\indo.nc -R%REGION% -J%SIZE% -Cgmt\batimetri.cpt -K -V -I+a0+nt0.5 -Y8 -P > %psfile%
psxy -R -JM -W1.0 -Sf0.4i/0.05ilt -Gblack -O -K  gmt\trench.gmt>> %psfile%

psxy %parameter% -R -JM -O -K -Sa0.2i -Wthin,red -Gred>> %psfile%
...
gawk "{ print $1+0.06,$2,$3-4,$4,$5,$6,$7}" %kota_malut% | pstext -R -JM -O -K >> %psfile%
gawk "{ print $1+0.06,$2,$3-4,$4,$5,$6,$7}" %kota_sulut% | pstext -R -JM -O -K >> %psfile%

psbasemap -J -R%REGION% -B%AXIS% --MAP_LABEL_OFFSET=0.2 --FONT_ANNOT_PRIMARY=9p --FONT_LABEL=11p,Heveltica --MAP_FRAME_TYPE=inside --MAP_FRAME_WIDTH=0.1p -P -K -O -V -Lf%scalelon%/%scalelat%/1/100+lKM  >> %psfile%
