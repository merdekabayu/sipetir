...
gmt grdimage fungsi/gmt/map_pgr.nc -R$REGION -J$SIZE -Cfungsi/gmt/batimetri.cpt -K -V -I+a0+nt0.5 -Y8 -P > $psfile
...
gmt psxy $kecamatan -R -JM -O -K -Sc0.1i -G0 >> $psfile
awk '{ print $1+0.06,$2,$3,$4,$5,$6,$7}' $kota_besar_malut | gmt pstext -R -JM -O -K >> $psfile
awk '{ print $1+0.04,$2,$3-4,$4,$5,$6,$7}' $kota_malut | gmt pstext -R -JM -O -K >> $psfile
awk '{ print $1+0.04,$2,$3-4,$4,$5,$6,$7}' $kota_sulut | gmt pstext -R -JM -O -K >> $psfile
awk '{ print $1+0.02,$2,$3-2,$4,$5,$6,$7,$8,$9,$10,$11}' $kecamatan | gmt pstext -R -JM -O -K >> $psfile
#awk '{print $1+0.12, $2, BL, $14}' tahun_focal.txt |

gmt psbasemap -J -R$REGION -B$AXIS --MAP_LABEL_OFFSET=0.2 --FONT_ANNOT_PRIMARY=9p --FONT_LABEL=11p,Heveltica --MAP_FRAME_TYPE=inside --MAP_FRAME_WIDTH=0.1p -P -K -O -V -Lf$skala/1/20+lKM  >> $psfile

gmt pscoast -Dl -R122/134/-3.2/3.6 -JM3.5c -W0.3,gray40 -Swhite -Ggray40 --MAP_FRAME_TYPE=inside -O -K -V -Blrbt -Y0.1 -X1.1>> $psfile
